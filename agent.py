...
file_writer = Agent(
    ...
    INSTRUCTIONS:
    - Create a marketable, contemporary movie title suggestion for the biography described in the PLOT_OUTLINE. If a title has been suggested in PLOT_OUTLINE, you can use it, or replace it with a better one.
...
screenwriter = Agent(
    name="screenwriter",
    model=model_name,
    description="As a screenwriter, write a logline and plot outline for a biopic about a cricket player.",
    instruction="""
    INSTRUCTIONS:
    Your goal is to write a logline and three-act plot outline for an inspiring movie about the cricket player described by the PROMPT: { PROMPT? }
